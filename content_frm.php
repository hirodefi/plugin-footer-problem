...
function _ProcessCont_Cp_diviStck( $ctx, &$ctxProcess, $doc, $xpath )
{
	...
	foreach( Gen::GetArrField( $cfg, array( '' ), array() ) as $id => $cfgItem )
	{
		...
		HtmlNd::InsertAfter( $item -> parentNode, $itemSticky, $item );
		...
		foreach( $aItemTop as $itemFirstSlideTmp )
		{
			...
			while( $itemSlideChild = ( $itemSlideChildNext ? $itemSlideChildNext : HtmlNd::GetNextTreeChild( $itemFirstSlideTmp, $itemSlideChild ) ) )
			{
				...
				if( $attrType != 'row' && $attrType != 'column' && !HtmlNd::FindUpBy( $itemSlideChild, function( $nd, $data ) { return( $nd -> nodeName == 'rs-column' ); } ) )
					...
					{
						$a = array(); foreach( ( array )(isset($attrDim[ 'w' ])?$attrDim[ 'w' ]:'auto') as $i => $v ) $a[ $i ][ 'white-space' ] = ( $v == 'auto' ) ? 'nowrap' : ( is_array( (isset($attrText[ 'w' ])?$attrText[ 'w' ]:null) ) ? (isset($attrText[ 'w' ][ $i ])?$attrText[ 'w' ][ $i ]:null) : (isset($attrText[ 'w' ])?$attrText[ 'w' ]:'nowrap') );
						_RevSld_SetStyleAttr( $styleSeparated, $aItemStyle, $itemChildSelector, $a );
					}
					...
					if( $srcSvg = $itemSlideChild -> getAttribute( 'data-svg_src' ) )
					{
						...
						if( $itemSvg = HtmlNd::ParseAndImport( $doc, $imgSrc -> GetCont() ) )
...
function _RevSld_GetPrmsFromScr( $item, $itemInitCmnScr )
{
	...
	if( $aCssCleanSelLate || $aCssCleanSel )
		$itemInitCmnScr -> nodeValue = substr_replace( $itemInitCmnScr -> nodeValue, ( $aCssCleanSelLate ? ( $posStartInit[ 1 ][ 0 ] . '.on( "revolution.slide.onloaded", function(){jQuery(this).children("' . implode( ',', $aCssCleanSelLate ) . '").remove();});' ) : '' ) . ( $aCssCleanSel ? ( $posStartInit[ 1 ][ 0 ] . '.on( "revolution.slide.afterdraw", function(){jQuery(this).children("' . implode( ',', $aCssCleanSel ) . '").remove();});' ) : '' ), $posStartInit[ 1 ][ 1 ], 0 );
...
function _ProcessCont_Cp_lottGen_AdjustItem( $ctx, &$ctxProcess, $settCache, $settImg, $settCdn, $doc, $xpath, $item, $renderer, &$srcData )
{
	...
	{
		if( !adkxsshiujqtfk( $ctxProcess, $settCache, 'json', $cont, $src ) )
			return( false );
